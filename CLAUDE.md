...
> 📋 **기능을 추가·변경하기 전에 `docs/SPEC.md`를 먼저 볼 것.**
> 코드·DB·피그마가 서로 어긋나 있어 그대로 두면 판단이 갈린다. 무엇을 만들지,
> 무엇이 아직 미결정인지(D1~D7), 우선순위가 거기에 정리돼 있다.
...
Phase 6 🚧  프로덕션 배포 — CD·이미지 완료. 호스팅만 남음 (지금 Render → 나중 Oracle Cloud VM)

## 배포 현황

**전체 배포 가이드: `deploy/DEPLOY.md`** (Render 즉시 배포 + Oracle 풀 세팅 체크리스트)
...
.github/workflows/deploy.yml    main push → amd64 빌드 → ghcr.io push  (✅ 검증됨)
deploy/docker-compose.yml       Oracle/자가서버: khunnect + Watchtower(자동 pull/교체)  OWNER=imtomyi 반영됨
deploy/DEPLOY.md                Render(지금) + Oracle(나중) 배포 절차
frontend/.env.example           환경변수 템플릿
```

✅ 완료: GitHub Secrets(`VITE_SUPABASE_URL`, `VITE_SUPABASE_ANON_KEY`) 등록 · deploy.yml OWNER=imtomyi ·
