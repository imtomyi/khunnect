...
✅ **자동 배포 연결됨.** ghcr는 Render로 webhook을 쏘지 않아 `:latest`를 덮어써도 Render가
변화를 감지하지 못하는 문제가 있었는데, `deploy.yml` 마지막에 **Render Deploy Hook 호출**
스텝을 넣어 해결했다(Secret: `RENDER_DEPLOY_HOOK`).
→ 이제 **`main`에 push하면 빌드 → ghcr push → Render 재배포까지 전부 자동**이다.
...
### 이미지 갱신 반영 — 자동 (설정 완료)
`main` push → `deploy.yml`이 빌드·ghcr push 후 **Deploy Hook을 호출**해 Render를 재배포한다.
따로 할 일 없음.

수동으로 재배포해야 할 때(예: 이미지 롤백):
Render 대시보드 → **Manual Deploy → Deploy latest reference**
